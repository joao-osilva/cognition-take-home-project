...
| 10  | Notifications                           | **Build (in-app)**  | NeonDB + Inngest + shadcn                      | `notifications` table (recipient, type, payload, read_at) written on approval/flag events; inbox + unread count in the shared shell. Slack/email delivery deferred (see known-gaps #8)       |

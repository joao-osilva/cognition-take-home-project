...
- Thin dispatcher interface: `notify(recipient, type, payload)` writes the `notifications` table; the shell shows an unread-count badge on the Inbox nav item, and `/inbox` (the landing page) gives every user a filterable, paginated history with per-item mark-read (a `defineAction()` scoped to the current actor) and deep links into the relevant app.
